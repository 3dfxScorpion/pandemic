		...
		while(!cityInfoFile.eof()) {
			...
			if(fileInput == "") {
				...
				// 
					...
					City currentCity = City(cityName, cityColor, population, black, blue, red, yellow, station);
...
string Map::infectedList() {
	...
	infectedCities << setw(15) << std::left << "City:" << setw(8) << "Black:" 
		<< setw(8) << "Blue:" << setw(8) << "Red:" << setw(8) << "Yellow:" << endl;
	for(int i = 0; i < worldMap.size(); i++) {
		if(worldMap[i].getInfectedBool()) {
			infectedCities << setw(15) << std::left << worldMap[i].getCityName() << setw(8) 
