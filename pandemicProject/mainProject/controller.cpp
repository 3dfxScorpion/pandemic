...
void do_move_to_city(Player* p);
void do_charter_flight(Player* p);
void do_direct_flight(Player* p);
void do_shuttle_flight(Player* p);
void do_build_station(Player* p);
void do_treat_disease(Player* p);
void do_cure_disease(Player* p);
void do_share_knowledge(Player* p);
// primary commands
string cmds01[] = {
    "Move To City",
    "Charter Flight",
...
void Controller::doProcessMenu(Player* p) {
    ...
    if ( choice < menu.getMappedFunctions().size() )
        menu.getMappedFunctions()[choice](p);
    switch (choice) {
      case 10 : cout << "\nGoodbye...\n"; exit(0);
    }
...
int Controller::run() {
    ...
    try {
        ...
        menu.setMappedFunctions(mainFunctions);
        menu.setMappedCubes("black", model.getCubeCount(black));
        menu.setMappedCubes("blue", model.getCubeCount(blue));
        menu.setMappedCubes("red", model.getCubeCount(red));
        menu.setMappedCubes("yellow", model.getCubeCount(yellow));
